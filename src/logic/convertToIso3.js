const isoCountries1 = [
  ...
  { ccode: 'LA', cname: 'Lao Peoples Democratic Republic' },
...
const getIsoCode = (countryName) => {
  if (countryName === 'jersey' || countryName === 'micronesia') return 'eg';
  const cName = countryName.toLowerCase();
  const country = isoCountries1.find((country) => country.cname.toLowerCase().includes(cName));
  if (cName.includes('korea')) return 'kr';
  return country ? country.ccode.toLowerCase() : 'za';
};
